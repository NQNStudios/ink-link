...
class StoryCommands {
	public static macro function build():Array<Field> {
		...
		var switchExp = ESwitch(macro(command : Array<String>), processCommandCases, macro {Sys.println('error! bad command $command'); return "";}).at();
		// trace(switchExp.toString());

		var processCommandFunction = {
			expr: // macro switch (command: Array<String>) { case ["fuck"]: Sys.println("yes"); },
			switchExp,
			args: [
				{
					name: "command"
